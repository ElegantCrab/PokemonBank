...
export const obtenerImagenPokemon = async (pokemonLista) => {
    ...
    console.log(pokemonNuevaLista);
    const result = await axios.all(pokemonLista.map(x => axios.get(obtenerPokemonString + (x.name ?? x.id))));
        result.forEach((resp, i) => {
        if(resp.status === 200){
            pokemonNuevaLista[i].id = resp.data.id;
            pokemonNuevaLista[i].name = resp.data.name;
...
export const obtenerEvolucionesPorEspecie = async (especieUrl) => {
    ...
    if(resp.status === 200){
        ...
        result = depthFirst(node => node.evolves_to)(evoluciones.chain)
        .map(({species}) => species.name);
        if(result){
            console.log(result);
            let evoluciones = [];
            for (const evolucion of result) {
                const resp = await obtenerPokemon(evolucion);
                evoluciones.push({ 
                    nombre: evolucion, 
